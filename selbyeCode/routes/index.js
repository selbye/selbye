...
// })
//sign up logic
router.post("/register", toLowerCase, function (req, res) {
                ...
                if (req.user.facebook == null) {
                    User.findOneAndUpdate({ 'google.id': res.locals.currentUser.google.id }, { 'username': req.body.username }).exec(function (err, founduser) {
                        if (err) {
                            ...
                            req.logout();
                            // req.flash("success", "Username updated. Please login again to continue")
                            // res.redirect("/")
...
router.get("/message/:id", isLoggedIn, function (req, res) {
